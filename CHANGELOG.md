...
- **Version 1.0.0** - Single-commit release (v1). Includes full feature set: AV1 encoding (SVT-AV1/libaom/rav1e), skip log (AV1 + larger), memory-limit with systemd-run fallback, hardware decode, and all options documented in README.
...
- **Memory limit:** If `systemd-run --scope` fails (e.g. "Access denied" when creating a transient scope without sufficient privileges), the script now disables the limit once at startup and continues without a memory cap instead of failing every job.
- **Documentation:** README updated to mention that on some environments the limit may be automatically disabled with a warning.
